...
def home_view(request):
    # products = models.Product.objects.filter(quantity__gt=F('reorder_level'))
    products = models.Product.objects.filter(qunatity__gt=F('reorderlevel'))
...
@login_required(login_url='adminlogin')
def admin_restock_view(request):
    products=models.Product.objects.filter(avaliblity=False)
    return render(request,'ecom/admin_restock.html',{'products':products})
...
def update_product_view(request,pk):
    product=models.Product.objects.get(id=pk)
    print('okk')
    productForm=forms.ADDProductForm(instance=product)
    if request.method=='POST':
        print('okk')
        productForm=forms.ADDProductForm(request.POST,request.FILES,instance=product)
        if productForm.is_valid():
            print('make')
            productForm.save()
            return redirect('admin-products')
...
@login_required(login_url='adminlogin')
def update_product_edit_stock(request,pk):
    product=models.Product.objects.get(id=pk)
    print('okk')
    productForm=forms.ProductFormup(instance=product)
    if request.method=='POST':
        print('okk')
        productForm=forms.ProductFormup(request.POST,request.FILES,instance=product)
        if productForm.is_valid():
            print('make')
            productForm.save()
            return redirect('admin-products')
    return render(request,'ecom/admin_update_productrestock.html',{'productForm':productForm})
    ...
    if 'product_ids' in request.COOKIES:
        ...
        if product_ids != "":
            ...
            for p in products:
                total=total+p.price
    return render(request,'ecom/cart.html',{'products':products,'total':total,'product_count_in_cart':product_count_in_cart})
def increace_quntity(request,id):
    product_id = str(id)  # Replace with the actual product_id
    if 'product_ids' in request.COOKIES:
        product_ids = request.COOKIES['product_ids']
        if product_ids != "":
            product_id_in_cart = product_ids.split('|')
            if product_id in product_id_in_cart:
                # Increase the quantity for the product
                response = HttpResponse()
                response.set_cookie('product_ids', '|'.join(product_id_in_cart))
                return response
    return redirect('cart')

def decreace_quntity(request,id):
    product_id = str(id)  # Replace with the actual product_id
    if 'product_ids' in request.COOKIES:
        product_ids = request.COOKIES['product_ids']
        if product_ids != "":
            product_id_in_cart = product_ids.split('|')
            if product_id in product_id_in_cart:
                # Decrease the quantity for the product
                response = HttpResponse()
                response.set_cookie('product_ids', '|'.join(product_id_in_cart))
                return response
    return redirect('cart')
...
def customer_address_view(request):
    ...
    if request.method == 'POST':
        ...
        if addressForm.is_valid():
            ...
            message = f'Thank you for your order! Your total is ${total}. We will ship your order to the following address:\n\n{address}\n\nIf you have any questions, please contact us at {email}.'
...
def shop_view(request, pk):
    product_instance = models.Product.objects.get(id=pk)
    context = {'product_instance': product_instance}
    return render(request,'ecom/shop.html', {'p': product_instance})
